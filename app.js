...
function initNavigation() {
  ...
  // Desktop inventory/challenges/codex/help/reset buttons (if they exist)
  document.getElementById('inventory-btn')?.addEventListener('click', () => {
    showNotification('Opening Inventory...', 1000);
    // Inventory modal handling
  });
  
  document.getElementById('challenges-btn')?.addEventListener('click', () => {
    showNotification('Viewing Badges...', 1000);
  });
  
  document.getElementById('codex-btn')?.addEventListener('click', () => {
    showNotification('Opening Codex...', 1000);
  });
  
  document.getElementById('help-btn')?.addEventListener('click', () => {
    showNotification('Showing Help...', 1000);
  });
  ...
  document.getElementById('save-btn')?.addEventListener('click', () => {
    saveGameState();
    showNotification('💾 Game saved!', 1500);
  });
...
// ========================================
// VIRTUAL JOYSTICK FOR MOBILE 3D CAVE NAVIGATION
// ========================================

function initVirtualJoystick() {
  const joystickElement = document.getElementById('cave-joystick');
  if (!joystickElement) return;

  const joystickBg = joystickElement.querySelector('.joystick-bg');
  ...
  // Get bounding rect for joystick background
  const getJoystickBounds = () => joystickBg.getBoundingClientRect();

  // Send movement command to cave iframe
  const sendMovement = (x, y) => {
    if (!caveFrame) {
      caveFrame = document.querySelector('#cave-iframe');
    }
    
    if (caveFrame && caveFrame.contentWindow) {
      caveFrame.contentWindow.postMessage({
        type: 'joystickMove',
        x: x,
        y: y
      }, '*');
  ...
  const calculatePosition = (touchX, touchY) => {
    ...
    // Joystick background radius
    const maxRadius = bounds.width / 2;
    
    // Calculate distance from center
    const distance = Math.sqrt(deltaX * deltaX + deltaY * deltaY);
    
    // Limit stick position to radius
    let limitedX = deltaX;
    let limitedY = deltaY;
    ...
    // Normalize to -1 to 1 range for sending to cave
  ...
  joystickBg.addEventListener('touchstart', (e) => {
    ...
    joystickStick.style.transform = `translate(${position.x}px, ${position.y}px)`;
    joystickStick.classList.add('active');
    
    sendMovement(position.normalizedX, position.normalizedY);
  });
...
function initMobileGameControls() {
  const controlButtons = document.querySelectorAll('.minigame-control-btn');
  ...
  controlButtons.forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      // Map direction to keyboard key for game compatibility
      const keyMap = {
        'up': 'ArrowUp',
        'down': 'ArrowDown',
        'left': 'ArrowLeft',
      ...
      // Simulate key press events for the game
      ...
      // Send to modal or document depending on context
...
// ========================================
// MOBILE TOOLBAR DRAWER FOR CAVEPAINTER
// ========================================

function initMobileToolbarDrawer() {
  const toggleBtn = document.querySelector('.mobile-toolbar-toggle');
  const drawer = document.querySelector('.mobile-toolbar-drawer');
  const caveFrame = document.querySelector('#cave-iframe');

  if (!toggleBtn || !drawer || !caveFrame) return;
